...
## 重要指令：输出规范
**严格要求：**
1. **直接输出分析内容**：不要任何开场白、介绍、问候或解释性文字
2. **禁止自我介绍**：不要说"作为专家"、"我将为您"、"好的"、"以下是"等任何AI身份介绍
3. **立即开始正文**：第一个字符必须是HTML标签，直接从 \`<div class="analysis-section">\` 开始
4. **纯净报告内容**：只输出与品牌分析报告相关的内容，任何无关文字都不要生成
5. **完整HTML结构**：确保输出完整的8个分析维度，每个维度都要有详实内容

现在直接开始输出品牌定位分析报告，第一个字符必须是 \`<\`：`;
    }
