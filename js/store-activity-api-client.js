...
class StoreActivityAPIClient {
    ...
    getAPIBaseURL(originalURL) {
        ...
        if (isServerlessPlatform || (protocol === 'https:' && !hostname.includes('.github.io'))) {
            console.log('[店铺活动] 环境检测: Serverless平台，使用API代理');
            return '/api/chat/completions';
        }

        // 3. GitHub Pages警告
        if (hostname.includes('.github.io')) {
            console.warn('[店铺活动] 环境检测: GitHub Pages，直连API（可能不稳定）');
            console.warn('[店铺活动] 建议: 部署到Vercel以获得更好的稳定性');
        }

        // 4. file://协议错误提示
        if (protocol === 'file:') {
            console.error('[店铺活动] 错误: 请勿直接打开HTML文件');
            console.error('[店铺活动] 请启动开发服务器: npm start');
        }

        // 默认：直连API
        console.log('[店铺活动] 环境检测: 直连API模式');
        return originalURL;
    }
